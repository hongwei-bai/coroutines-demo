...
class DemoViewModelCascade : ViewModel() {
    ...
    private suspend fun fetchCascade() {
        val content = NetworkClient.demoApi.getContentBody()
        val rate = NetworkClient.demoApi.getRate()
        Log.d("bbbb", "display: ${RateUtil.toDisplay(content.content, rate.rate)}")
    }
}